...
RUN sed -i 's|;\?date.timezone =.*|date.timezone = ${DATE_TIMEZONE}|' /etc/php/7.2/fpm/php.ini \
    && sed -i 's|;\?date.timezone =.*|date.timezone = ${DATE_TIMEZONE}|' /etc/php/7.2/cli/php.ini \
    && sed -i 's|max_execution_time =.*|max_execution_time = ${REQUEST_TIMEOUT}|' /etc/php/7.2/fpm/php.ini \
    && sed -i 's|;\?request_terminate_timeout =.*|request_terminate_timeout = ${REQUEST_TIMEOUT}|' /etc/php/7.2/fpm/pool.d/www.conf \
    && sed -i 's|max_input_time =.*|max_input_time = ${MAX_INPUT_TIME}|' /etc/php/7.2/fpm/php.ini \
    && sed -i 's|memory_limit =.*|memory_limit = ${MEMORY_LIMIT}|' /etc/php/7.2/fpm/php.ini \
    && sed -i 's|upload_max_filesize =.*|upload_max_filesize = ${POST_MAX_SIZE}|' /etc/php/7.2/fpm/php.ini \
    && sed -i 's|post_max_size =.*|post_max_size = ${POST_MAX_SIZE}|' /etc/php/7.2/fpm/php.ini \
    ...
    && sed -i 's|;\?pm.status_path =.*|pm.status_path = /status|' /etc/php/7.2/fpm/pool.d/www.conf \
    ...
    && sed -i 's|;\?php_admin_flag\[log_errors\] =.*|php_admin_flag\[log_errors\] = on|' /etc/php/7.2/fpm/pool.d/www.conf
