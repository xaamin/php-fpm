...
RUN sed -i 's|;\?date.timezone =.*|date.timezone = ${DATE_TIMEZONE}|' /etc/php/8.1/fpm/php.ini \
    && sed -i 's|;\?date.timezone =.*|date.timezone = ${DATE_TIMEZONE}|' /etc/php/8.1/cli/php.ini \
    && sed -i 's|max_execution_time =.*|max_execution_time = ${REQUEST_TIMEOUT}|' /etc/php/8.1/fpm/php.ini \
    && sed -i 's|;\?request_terminate_timeout =.*|request_terminate_timeout = ${REQUEST_TIMEOUT}|' /etc/php/8.1/fpm/pool.d/www.conf \
    && sed -i 's|max_input_time =.*|max_input_time = ${MAX_INPUT_TIME}|' /etc/php/8.1/fpm/php.ini \
    && sed -i 's|memory_limit =.*|memory_limit = ${MEMORY_LIMIT}|' /etc/php/8.1/fpm/php.ini \
    && sed -i 's|upload_max_filesize =.*|upload_max_filesize = ${POST_MAX_SIZE}|' /etc/php/8.1/fpm/php.ini \
    && sed -i 's|post_max_size =.*|post_max_size = ${POST_MAX_SIZE}|' /etc/php/8.1/fpm/php.ini \
    ...
    && sed -i 's|;\?listen.allowed_clients =.*|;listen.allowed_clients =|' /etc/php/8.1/fpm/pool.d/www.conf \
    && sed -i 's|;\?catch_workers_output =.*|catch_workers_output = yes|' /etc/php/8.1/fpm/pool.d/www.conf \
    && sed -i 's|;\?php_admin_flag\[log_errors\] =.*|php_admin_flag\[log_errors\] = on|' /etc/php/8.1/fpm/pool.d/www.conf
