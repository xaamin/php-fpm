...
sed -i 's|error_log =.*|error_log = '${LOGS}'/php-fpm.log|' /etc/php/7.0/fpm/php-fpm.conf || true
sed -i 's|;\?php_admin_value\[error_log\] =.*|php_admin_value\[error_log\] = '${LOGS}'/php-fpm.www.log|' /etc/php/7.0/fpm/pool.d/www.conf || true
